-- Repair Circle Momentum / Weekly Proof Sprint policies.
--
-- RLS policies call this helper while evaluating rows. If authenticated users do
-- not have EXECUTE permission on the helper, Postgres can fail the whole query
-- with "permission denied for function is_circle_connection", even for a user's
-- own sprint row. Keep the helper outside the exposed public schema, but grant
-- authenticated enough privilege for RLS evaluation.
