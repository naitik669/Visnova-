-- Move the Circle helper used by accountability policies out of the exposed public schema.
-- Public RPC access is not needed for this helper; policies call the private function directly.
...
revoke all on function app_private.is_circle_connection(uuid) from authenticated;
...
revoke all on function public.is_circle_connection(uuid) from public;
revoke all on function public.is_circle_connection(uuid) from anon;
revoke all on function public.is_circle_connection(uuid) from authenticated;
