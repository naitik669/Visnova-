...
@keyframes vnCheckboxComplete { 0%,18% { fill:none; stroke:#A78BFA; stroke-width:2; } 24%,85% { fill:#F7F3FF; stroke:#8B5CF6; stroke-width:2.5; } 92%,100% { fill:none; stroke:#A78BFA; stroke-width:2; } }
...
@keyframes vnSlideToggle { 0%,20% { transform:translateX(0); fill:#8B5CF6; } 28%,85% { transform:translateX(20px); fill:#fff; } 92%,100% { transform:translateX(0); fill:#8B5CF6; } }
@keyframes vnColorToggle { 0%,20% { fill:#F7F3FF; stroke:#E7DDFF; } 28%,85% { fill:#8B5CF6; stroke:#8B5CF6; } 92%,100% { fill:#F7F3FF; stroke:#E7DDFF; } }
...
@keyframes vnLineBreakOuter { 0%,25% { stroke:#8B5CF6; stroke-dasharray:none; opacity:1; } 33%,58% { stroke:#A78BFA; stroke-dasharray:4 4; opacity:.7; } 66%,85% { stroke:#8B5CF6; stroke-dasharray:none; opacity:1; } 92%,100% { opacity:0; } }
@keyframes vnLineBreakCenter { 0%,25% { stroke:#8B5CF6; stroke-dasharray:none; opacity:.8; transform:scaleX(1); } 33%,58% { stroke:#FFE0E8; stroke-dasharray:2 2; opacity:.3; transform:scaleX(.5); } 66%,85% { stroke:#8B5CF6; stroke-dasharray:none; opacity:.8; transform:scaleX(1); } 92%,100% { opacity:0; } }
...
@keyframes vnThemeShift { 0%,15%,100% { --vn-theme-bg:#F7F3FF; --vn-theme-primary:#8B5CF6; --vn-theme-secondary:#A78BFA; --vn-theme-card:#fff; --vn-theme-border:#E7DDFF; --vn-theme-text:#25163D; --active-index:0; } 20%,35% { --vn-theme-bg:#F0F6FF; --vn-theme-primary:#3B82F6; --vn-theme-secondary:#60A5FA; --vn-theme-card:#fff; --vn-theme-border:#DBEAFE; --vn-theme-text:#1E3A8A; --active-index:1; } 40%,55% { --vn-theme-bg:#FFFDF5; --vn-theme-primary:#D97706; --vn-theme-secondary:#FBBF24; --vn-theme-card:#fff; --vn-theme-border:#FEF3C7; --vn-theme-text:#78350F; --active-index:2; } 60%,75% { --vn-theme-bg:#F0FDF4; --vn-theme-primary:#10B981; --vn-theme-secondary:#34D399; --vn-theme-card:#fff; --vn-theme-border:#DCFCE7; --vn-theme-text:#064E3B; --active-index:3; } 80%,95% { --vn-theme-bg:#110C1D; --vn-theme-primary:#9061FF; --vn-theme-secondary:#C3B4FC; --vn-theme-card:#1C152B; --vn-theme-border:#2B2142; --vn-theme-text:#fff; --active-index:4; } }
...
@keyframes vnWaveExpand { 15% { transform:scale(0); opacity:0; fill:#3B82F6; } 17% { transform:scale(1.6); opacity:.15; } 20%,35% { transform:scale(0); opacity:0; fill:#D97706; } 37% { transform:scale(1.6); opacity:.15; } 40%,55% { transform:scale(0); opacity:0; fill:#10B981; } 57% { transform:scale(1.6); opacity:.15; } 60%,75% { transform:scale(0); opacity:0; fill:#9061FF; } 77% { transform:scale(1.6); opacity:.15; } 80%,95% { transform:scale(0); opacity:0; fill:#8B5CF6; } 97% { transform:scale(1.6); opacity:.15; } 100% { transform:scale(0); opacity:0; } }
...
function IconCard({ x, y, label, icon }: { x: number; y: number; label: string; icon: 'eye' | 'check' | 'bar' | 'note' }) {
  return (
    <g transform={`translate(${x} ${y})`}>
      <rect width="140" height="64" rx="16" fill="url(#vnCardGrad)" stroke="#E9D5FF" strokeWidth="1.5" />
      <circle cx="28" cy="32" r="12" fill="#8B5CF6" fillOpacity="0.1" />
      {icon === 'eye' && <><path d="M22 32 C22 32 24 28 28 28 C32 28 34 32 34 32 C34 32 32 36 28 36 C24 36 22 32 22 32 Z" stroke="#8B5CF6" strokeWidth="1.8" strokeLinecap="round" /><circle cx="28" cy="32" r="2" fill="#8B5CF6" /></>}
      {icon === 'check' && <path d="M24 32 L27 35 L33 29" stroke="#8B5CF6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />}
      {icon === 'bar' && <path d="M21 37 L21 32 M28 37 L28 27 M35 37 L35 30" stroke="#8B5CF6" strokeWidth="2" strokeLinecap="round" />}
      {icon === 'note' && <path d="M23 27 H33 M23 32 H33 M23 37 H29" stroke="#8B5CF6" strokeWidth="1.8" strokeLinecap="round" />}
      <text x="50" y="37" fontFamily="system-ui, sans-serif" fontWeight="600" fontSize="13" fill="#25163D">{label}</text>
    </g>
  );
}
  ...
  return (
    <defs>
      ...
      <filter id="vnSoftShadow" x="-20%" y="-20%" width="140%" height="140%">
        <feDropShadow dx="0" dy="8" stdDeviation="10" floodColor="#8B5CF6" floodOpacity="0.08" />
      </filter>
    </defs>
  );
}
  ...
  return (
    <svg viewBox="0 0 800 460" width="100%" height="100%" fill="none" role="img" aria-label="Lost in Vision space">
      ...
      <g className="vn-compass-search">
        <path d="M400 230 L190 140 M400 230 L550 120" stroke="#8B5CF6" strokeWidth="1.5" strokeDasharray="4 4" className="vn-search-rays" />
        <circle cx="400" cy="230" r="75" stroke="#A78BFA" strokeWidth="1.5" strokeDasharray="6 6" className="vn-orbit-outer" />
        <circle cx="400" cy="230" r="48" stroke="#8B5CF6" strokeWidth="2" strokeDasharray="40 80" className="vn-orbit-inner" />
        <circle cx="400" cy="230" r="16" fill="url(#vnBrandGrad)" filter="url(#vnGlow)" />
      </g>
      <g className="vn-state-404">
        <rect x="230" y="110" width="340" height="240" rx="28" fill="#FFFFFF" fillOpacity="0.75" stroke="#F1ECFF" strokeWidth="2" filter="url(#vnGlow)" />
        <path d="M315 160 L270 245 H340 M315 160 V290" stroke="#8B5CF6" strokeWidth="14" strokeLinecap="round" strokeLinejoin="round" className="vn-draw-digit" />
        ...
        <path d="M515 160 L470 245 H540 M515 160 V290" stroke="#8B5CF6" strokeWidth="14" strokeLinecap="round" strokeLinejoin="round" className="vn-draw-digit" />
        <circle cx="400" cy="225" r="10" fill="#8B5CF6" className="vn-sparkle" />
        <circle cx="345" cy="180" r="4" fill="#A78BFA" className="vn-sparkle" />
        <circle cx="460" cy="275" r="5" fill="#8B5CF6" className="vn-sparkle vn-s2" />
      </g>
    </svg>
  );
}
...
function ProgressEmptySvg() {
  return (
    <svg viewBox="0 0 800 360" width="100%" height="100%" fill="none" role="img" aria-label="Progress proof timeline">
      <BaseDefs />
      <circle cx="260" cy="180" r="90" fill="#E7DDFF" opacity="0.4" filter="blur(35px)" />
      <path className="vn-timeline-path" d="M160 250 H640" stroke="#E7DDFF" strokeWidth="4" strokeLinecap="round" />
      <circle className="vn-node vn-node-2" cx="410" cy="250" r="5" fill="#FFF" stroke="#E7DDFF" strokeWidth="3" />
      <circle className="vn-node vn-node-3" cx="560" cy="250" r="5" fill="#FFF" stroke="#E7DDFF" strokeWidth="3" />
      <path className="vn-connector-line" d="M260 250 V165" stroke="#A78BFA" strokeWidth="1.5" strokeLinecap="round" strokeDasharray="3 3" />
      <g className="vn-active-dot">
        <circle className="vn-dot-ripple" cx="260" cy="250" r="22" stroke="#8B5CF6" strokeWidth="1.5" fill="none" />
        <circle cx="260" cy="250" r="10" fill="#8B5CF6" filter="url(#vnGlow)" opacity="0.5" />
        <circle cx="260" cy="250" r="6" fill="#8B5CF6" stroke="#FFF" strokeWidth="2" />
      </g>
      <g className="vn-proof-card">
        <rect x="170" y="80" width="180" height="84" rx="16" fill="url(#vnCardGrad)" stroke="#E7DDFF" filter="url(#vnSoftShadow)" />
        <g className="vn-check-circle">
          <circle cx="215" cy="122" r="14" fill="#F7F3FF" stroke="#A78BFA" />
          <path className="vn-check-path" d="M209 122 L213 126 L221 117" stroke="#8B5CF6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </g>
        <rect x="242" y="114" width="75" height="7" rx="3.5" fill="#8B5CF6" fillOpacity="0.8" />
        <rect x="242" y="128" width="45" height="5" rx="2.5" fill="#A78BFA" fillOpacity="0.5" />
      </g>
    </svg>
  );
}
...
function VisionCreatedSvg() {
  return (
    <svg viewBox="0 0 800 440" width="100%" height="100%" fill="none" role="img" aria-label="Vision created">
      <BaseDefs />
      <circle cx="400" cy="240" r="100" fill="#F1ECFF" opacity="0.3" filter="blur(40px)" />
      <circle className="vn-orbit-path" cx="400" cy="240" r="64" stroke="#A78BFA" strokeWidth="2" strokeDasharray="6 6" />
      <circle className="vn-seed" cx="400" cy="240" r="12" fill="#8B5CF6" filter="url(#vnGlow)" />
      <g className="vn-vision-card">
        <rect x="270" y="150" width="260" height="150" rx="24" fill="#FFF" stroke="#F1ECFF" strokeWidth="1.5" filter="url(#vnSoftShadow)" />
        <g className="vn-card-detail">
          <circle cx="320" cy="225" r="18" fill="#F1ECFF" />
          <circle cx="320" cy="225" r="11" stroke="#8B5CF6" strokeWidth="2.5" />
          <circle cx="320" cy="225" r="4" fill="#A78BFA" />
          <circle cx="338" cy="208" r="9" fill="#8B5CF6" />
          <path className="vn-check-path" d="M334 208 L337 211 L342 205" stroke="#FFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </g>
        <g className="vn-skeletal-line">
          <rect x="355" y="206" width="120" height="8" rx="4" fill="#8B5CF6" />
          <rect x="355" y="222" width="80" height="6" rx="3" fill="#A78BFA" opacity="0.6" />
        </g>
      </g>
      <g className="vn-mini-tasks"><rect x="190" y="110" width="95" height="42" rx="12" fill="#FFF" stroke="#F1ECFF" filter="url(#vnSoftShadow)" /><text x="225" y="135" fontFamily="system-ui" fontWeight="600" fontSize="10" fill="#25163D">Tasks</text></g>
      <g className="vn-mini-proof"><rect x="180" y="280" width="95" height="42" rx="12" fill="#FFF" stroke="#F1ECFF" filter="url(#vnSoftShadow)" /><text x="215" y="305" fontFamily="system-ui" fontWeight="600" fontSize="10" fill="#25163D">Proof</text></g>
      <g className="vn-mini-resources"><rect x="510" y="190" width="115" height="42" rx="12" fill="#FFF" stroke="#F1ECFF" filter="url(#vnSoftShadow)" /><text x="545" y="215" fontFamily="system-ui" fontWeight="600" fontSize="10" fill="#25163D">Resources</text></g>
    </svg>
  );
}
...
function TaskCompletedSvg() {
  return (
    <svg viewBox="0 0 800 360" width="100%" height="100%" fill="none" role="img" aria-label="Task completed">
      <BaseDefs />
      <circle cx="240" cy="180" r="120" fill="#F1ECFF" opacity="0.3" filter="blur(45px)" />
      <g className="vn-done-column">
        <rect x="470" y="70" width="230" height="230" rx="20" fill="none" stroke="#E7DDFF" strokeWidth="2" strokeDasharray="6 5" />
        <rect x="490" y="90" width="75" height="24" rx="12" fill="#E7DDFF" />
        <text x="527" y="106" fontFamily="system-ui" fontWeight="700" fontSize="11" fill="#8B5CF6" textAnchor="middle">DONE</text>
      </g>
      <g opacity="0.4">
        <rect x="100" y="70" width="230" height="230" rx="20" fill="none" stroke="#E7DDFF" strokeWidth="1.5" />
        <text x="162" y="106" fontFamily="system-ui" fontWeight="700" fontSize="11" fill="#25163D" textAnchor="middle">TO DO</text>
      </g>
      <g className="vn-task-card">
        <rect x="115" y="140" width="200" height="74" rx="16" fill="#FFF" stroke="#F1ECFF" filter="url(#vnSoftShadow)" />
        <circle className="vn-checkbox" cx="150" cy="177" r="11" />
        <path className="vn-check-path" d="M144.5 177 L148 180.5 L155.5 172.5" stroke="#8B5CF6" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        <rect x="175" y="168" width="95" height="7" rx="3.5" fill="#25163D" fillOpacity="0.8" />
        <rect x="175" y="181" width="60" height="5" rx="2.5" fill="#A78BFA" fillOpacity="0.5" />
      </g>
    </svg>
  );
}
...
function ProgressLoaderSvg() {
  return (
    <svg viewBox="0 0 500 500" width="100%" height="100%" fill="none" role="img" aria-label="Calculating progress">
      <BaseDefs />
      <circle cx="250" cy="250" r="110" fill="#F1ECFF" opacity="0.4" filter="blur(35px)" />
      <circle className="vn-inner-aura" cx="250" cy="250" r="68" fill="#8B5CF6" />
      <circle cx="250" cy="250" r="80" stroke="#DDD6FE" strokeWidth="5" opacity="0.6" />
      <circle className="vn-progress-ring" cx="250" cy="250" r="80" stroke="#8B5CF6" strokeWidth="7" strokeLinecap="round" />
      <circle cx="250" cy="250" r="130" stroke="#DDD6FE" strokeWidth="1.5" strokeDasharray="4 6" opacity="0.5" />
      <g className="vn-orbiting-system">
        {[[250,120,'M246 116 H254 M246 120 H254 M246 124 H250'],[380,250,'M375 250 L378 253 L384 247'],[250,380,'M250 376.5 V380 H253'],[120,250,'M116 246 H124 V254 H116 Z']].map(([cx, cy, path], index) => (
          <g className="vn-orbit-icon" key={index}>
            <circle className="vn-float-card" cx={Number(cx)} cy={Number(cy)} r="18" fill="#FFF" stroke="#A78BFA" filter="url(#vnSoftShadow)" />
            <path d={String(path)} stroke="#8B5CF6" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
          </g>
        ))}
      </g>
    </svg>
  ...
  return (
    <svg viewBox="100 70 280 200" width="100%" height="100%" fill="none" role="img" aria-label="Cookie choices">
      ...
      <g className="vn-cookie-group">
        <circle cx="210" cy="160" r="50" fill="#E5C3A6" stroke="#D4A276" strokeWidth="1.5" filter="url(#vnSoftShadow)" />
        <rect x="182" y="132" width="11" height="8" rx="4" fill="#8B5CF6" transform="rotate(-15 182 132)" />
        <rect x="228" y="130" width="14" height="10" rx="5" fill="#A78BFA" transform="rotate(10 228 130)" />
        <circle cx="206" cy="154" r="5" fill="#8B5CF6" />
      </g>
      <g className="vn-shield-group">
        <path d="M265 175 H305 V195 C305 210 285 220 285 220 C285 220 265 210 265 195 Z" fill="#A78BFA" stroke="#E7DDFF" strokeWidth="1.5" />
        <path className="vn-check-path" d="M276 195 L282 201 L294 189" stroke="#FFF" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      </g>
...
function PrivacySvg() {
  return (
    <svg viewBox="0 0 500 320" width="100%" height="100%" fill="none" role="img" aria-label="Private logs">
      <BaseDefs />
      <circle cx="250" cy="160" r="100" fill="#F1ECFF" opacity="0.4" filter="blur(35px)" />
      <g className="vn-log-card"><rect x="155" y="110" width="190" height="90" rx="18" fill="#FFF" stroke="#E7DDFF" filter="url(#vnSoftShadow)" /><rect x="180" y="134" width="115" height="7" rx="3.5" fill="#25163D" fillOpacity="0.8" /><rect x="180" y="148" width="65" height="5" rx="2.5" fill="#A78BFA" fillOpacity="0.5" /></g>
      <g className="vn-private-pill"><rect x="205" y="75" width="90" height="22" rx="11" fill="#8B5CF6" /><text x="250" y="89" fontFamily="system-ui" fontWeight="700" fontSize="10" fill="#FFF" textAnchor="middle">PRIVATE</text></g>
      <path className="vn-shield-outline" d="M130 90 H370 V155 C370 210 250 250 250 250 C250 250 130 210 130 155 Z" stroke="#8B5CF6" strokeWidth="2.5" fill="#8B5CF6" fillOpacity="0.04" />
      <g className="vn-lock-group"><path d="M238 150 V139 C238 132.5 243.5 127 250 127 C256.5 127 262 132.5 262 139 V150" stroke="#8B5CF6" strokeWidth="3" strokeLinecap="round" /><rect x="232" y="148" width="36" height="26" rx="6" fill="#8B5CF6" /></g>
      <g className="vn-check-badge"><circle cx="250" cy="160" r="18" fill="#8B5CF6" /><path className="vn-check-path" d="M242 160 L247 165 L257 155" stroke="#FFF" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></g>
    </svg>
  );
}
...
function ErrorSvg() {
  return (
    <svg viewBox="0 0 500 320" width="100%" height="100%" fill="none" role="img" aria-label="Connection retry">
      <BaseDefs />
      <circle cx="250" cy="160" r="90" fill="#FFE0E8" opacity="0.3" filter="blur(35px)" />
      <rect x="110" y="80" width="280" height="160" rx="24" fill="#FFF" stroke="#F1ECFF" filter="url(#vnSoftShadow)" />
      ...
      <g className="vn-orbit-group"><circle cx="250" cy="160" r="22" stroke="#A78BFA" strokeWidth="1.5" strokeDasharray="4 4" /><circle cx="250" cy="160" r="7" fill="#8B5CF6" /></g>
      <g className="vn-alert-indicator"><path d="M250 102 L259 118 C260 120 259 122 257 122 H243 C241 122 240 120 241 118 Z" fill="#FFE0E8" stroke="#8B5CF6" /><line x1="250" y1="110" x2="250" y2="115" stroke="#8B5CF6" strokeWidth="1.5" strokeLinecap="round" /></g>
      <g className="vn-retry-arrow"><circle cx="250" cy="160" r="20" fill="#F1ECFF" stroke="#E7DDFF" /><path d="M250 148 A12 12 0 1 1 240 153 M240 144 L240 153 H249" stroke="#8B5CF6" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></g>
    </svg>
  );
}
...
function CircleEmptySvg() {
  return (
    <svg viewBox="0 0 500 320" width="100%" height="100%" fill="none" role="img" aria-label="Circle empty">
      <BaseDefs />
      <circle cx="250" cy="160" r="64" stroke="#E7DDFF" strokeWidth="1.5" strokeDasharray="4 6" opacity="0.5" />
      <circle className="vn-pulse-ring" cx="250" cy="160" r="80" stroke="#A78BFA" strokeWidth="2" />
      <g className="vn-triangle-system">
        <path className="vn-connection-line" d="M250 115 L195 195" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" />
        <path className="vn-connection-line" d="M195 195 L305 195" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" />
        <path className="vn-connection-line" d="M305 195 L250 115" stroke="#A78BFA" strokeWidth="2" strokeLinecap="round" />
        <g className="vn-avatar-1"><circle cx="250" cy="115" r="16" fill="#8B5CF6" /><circle cx="250" cy="110" r="5" fill="#FFF" /><path d="M241 126 C241 121 245 118 250 118 C255 118 259 121 259 126 Z" fill="#FFF" /></g>
        <g className="vn-avatar-2"><circle cx="195" cy="195" r="16" fill="#CFFAEA" /><circle cx="195" cy="190" r="5" fill="#8B5CF6" /><path d="M186 206 C186 201 190 198 195 198 C200 198 204 201 204 206 Z" fill="#8B5CF6" /></g>
        <g className="vn-avatar-3"><circle cx="305" cy="195" r="16" fill="#A78BFA" /><circle cx="305" cy="190" r="5" fill="#FFF" /><path d="M296 206 C296 201 300 198 305 198 C310 198 314 201 314 206 Z" fill="#FFF" /></g>
      </g>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 500 320" width="100%" height="100%" fill="none" role="img" aria-label="Report sent">
      ...
      <g className="vn-inbox-system">
        <path d="M175 155 H325 V185 H175 Z" fill="#E7DDFF" opacity="0.5" />
        <g className="vn-feedback-card"><rect x="195" y="125" width="110" height="70" rx="14" fill="#FFF" stroke="#E7DDFF" /><rect x="210" y="139" width="80" height="6" rx="3" fill="#8B5CF6" /><rect x="210" y="151" width="55" height="5" rx="2.5" fill="#A78BFA" opacity="0.6" /></g>
        <g className="vn-inbox-front"><path d="M170 170 H215 C220 170 225 180 250 180 C275 180 280 170 285 170 H330 V200 C330 208 322 208 322 208 H178 C178 208 170 208 170 200 Z" fill="url(#vnCardGrad)" stroke="#A78BFA" strokeWidth="1.2" filter="url(#vnSoftShadow)" /></g>
        <g className="vn-check-badge"><circle cx="250" cy="115" r="18" fill="#8B5CF6" /><path className="vn-check-path" d="M242 115 L247 120 L257 110" stroke="#FFF" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></g>
      </g>
    </svg>
  );
}
...
function WaitlistSvg() {
  return (
    <svg viewBox="0 0 500 320" width="100%" height="100%" fill="none" role="img" aria-label="Waitlist joined">
      <BaseDefs />
      <path className="vn-gate-glow" d="M215 210 V135 C215 110 285 110 285 135 V210 Z" fill="#8B5CF6" fillOpacity="0.25" filter="url(#vnGlow)" />
      <path className="vn-gate-left" d="M215 210 V135 C215 110 250 110 250 115 V210 Z" fill="#FFF" stroke="#A78BFA" strokeWidth="1.5" />
      <path className="vn-gate-right" d="M250 210 V115 C250 110 285 110 285 135 V210 Z" fill="#FFF" stroke="#A78BFA" strokeWidth="1.5" />
      <g className="vn-invite-card">
        <path d="M150 110 H350 V155 A10 10 0 0 0 350 175 V220 H150 V175 A10 10 0 0 0 150 155 Z" fill="#FFF" stroke="#E7DDFF" filter="url(#vnSoftShadow)" />
        <path d="M235 110 V220" stroke="#E7DDFF" strokeWidth="1.5" strokeDasharray="4 4" />
        <circle cx="194" cy="165" r="14" stroke="#A78BFA" strokeWidth="1.2" strokeDasharray="3 3" />
        <circle cx="194" cy="165" r="5" fill="#8B5CF6" />
        <circle className="vn-progress-dot" cx="170" cy="195" r="3" fill="#8B5CF6" />
        <circle className="vn-progress-dot vn-dot-2" cx="170" cy="195" r="3" fill="#A78BFA" />
        <circle className="vn-progress-dot vn-dot-3" cx="170" cy="195" r="3" fill="#DDD6FE" />
        <g className="vn-check-badge"><circle cx="292" cy="165" r="13" fill="#8B5CF6" /><path className="vn-check-path" d="M285 165 L289 169 L299 161" stroke="#FFF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></g>
      </g>
    </svg>
  );
}
...
function ThemeSvg() {
  return (
    <svg className="vn-theme-engine" viewBox="0 0 500 320" width="100%" height="100%" fill="none" role="img" aria-label="Theme preview">
      ...
      {[170, 210, 250, 290, 330].map((cx, index) => <circle key={cx} cx={cx} cy="255" r="7" fill={['#8B5CF6', '#3B82F6', '#D97706', '#10B981', '#818CF8'][index]} />)}
