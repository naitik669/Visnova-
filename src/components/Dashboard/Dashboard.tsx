...
function DashboardActivityChart({ data }: { data: Array<{ name: string; output: number }> }) {
  ...
  const linePath = points.reduce((path, point, index) => {
    ...
    const controlOneX = previous.x + ((point.x - beforePrevious.x) / 6) * smoothing;
    const controlOneY = previous.y + ((point.y - beforePrevious.y) / 6) * smoothing;
    const controlTwoX = point.x - ((next.x - previous.x) / 6) * smoothing;
    const controlTwoY = point.y - ((next.y - previous.y) / 6) * smoothing;
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-full w-full overflow-visible" preserveAspectRatio="none" role="img" aria-label="Weekly output activity chart">
      ...
      <path d={linePath} fill="none" stroke="var(--accent)" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />
      {points.map((point) => (
        <g key={point.name}>
